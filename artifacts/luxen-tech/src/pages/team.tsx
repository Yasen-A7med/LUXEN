...
export default function Team() {
  ...
  const editForm = useForm<EditForm>({
    ...
    defaultValues: { name: "", jobTitle: "", department: "", email: "", phone: "", password: "", joinDate: "" },
  });
  ...
  const openEdit = (emp: Employee) => {
    setEditEmp(emp);
    editForm.reset({ name: emp.name, jobTitle: emp.jobTitle, department: emp.department, email: emp.email, phone: emp.phone, joinDate: emp.joinDate || "", password: "" });
  };

  const onEdit = (data: EditForm) => {
    if (!editEmp) return;
    const update: Partial<Employee & { password?: string }> = { name: data.name, jobTitle: data.jobTitle, department: data.department, email: data.email, phone: data.phone, joinDate: data.joinDate };
